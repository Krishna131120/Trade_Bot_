...
class FyersDataService:
    ...
    async def startup_event(self):
        """Startup event handler - supports mock mode"""
        try:
            ...
            if not connection_success:
                if self.allow_mock_mode:
                    ...
                    # Don't raise - allow mock mode to proceed
                else:
            ...
            # Start data updates (works in both real and mock mode)
            await self.start_data_updates()
            logger.info("Fyers Data Service started successfully")
        except Exception as e:
            logger.error(f"Startup error: {e}")
            if not self.allow_mock_mode:
                raise  # Re-raise if mock mode not enabled
            else:
                logger.warning("Continuing in mock mode despite startup error")
                await self.start_data_updates()
