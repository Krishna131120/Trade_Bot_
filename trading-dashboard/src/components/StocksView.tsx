...
const StocksView = ({
  ...
  return (
    <div className="space-y-4">
      <SessionDisclaimer />
      <div>
        <h2 className={`text-xl font-bold ${isLight ? 'text-gray-900' : isSpace ? 'text-white drop-shadow-lg' : 'text-white'
          } mb-1 flex items-center gap-2`}>
          <TrendingUp className={`w-5 h-5 ${isLight ? 'text-blue-600' : isSpace ? 'text-blue-400 drop-shadow' : 'text-blue-400'
            }`} />
          Stocks Market
        </h2>
        <p className={`${isLight ? 'text-gray-600' : isSpace ? 'text-gray-300' : 'text-gray-400'
      ...
      <div className={`${isLight
        ...
        {/* Progress indicator */}
        {progress && (
          <div className={`mb-3 p-3 ${isLight
            ...
            <div className="flex items-center gap-3">
              <Loader2 className={`w-5 h-5 animate-spin ${isLight ? 'text-blue-600' : 'text-blue-400'
                }`} />
              <div className="flex-1">
                <div className="flex justify-between mb-1">
                  ...
                  <span className={`text-sm ${isLight ? 'text-blue-600' : 'text-blue-400'
                    }`}>
                    {progress.progress}%
                  </span>
                </div>
                <div className={`w-full ${isLight ? 'bg-blue-200/50' : 'bg-blue-900/50'
                  } rounded-full h-2`}>
                  <div
                    className={`h-2 rounded-full transition-all duration-300 ${isLight ? 'bg-blue-500' : 'bg-blue-500'
                      }`}
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-3">
          <div className="flex-1 relative w-full">
            <Search className={`absolute left-2.5 top-1/2 transform -translate-y-1/2 ${isLight ? 'text-gray-500' : 'text-gray-400'
            ...
            <SymbolAutocomplete
              ...
              className="pl-8 pr-3 py-1.5 text-sm w-full"
            />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 sm:pb-0 hide-scrollbar">
            {onHorizonChange && (
              <select
                ...
                className={`px-2.5 py-1.5 text-sm ${isLight
            ...
            <button
              ...
              className={`px-3 py-1.5 ${isLight
                  ? 'bg-blue-500 hover:bg-blue-600 text-white'
                  : isSpace
                    ? 'bg-blue-600 hover:bg-blue-700 text-white'
                    : 'bg-blue-500 hover:bg-blue-600 text-white'
                } text-sm font-semibold rounded transition-all disabled:opacity-50 flex items-center gap-1.5 whitespace-nowrap`}
            >
      ...
      {error && (
        <div className={`${isLight
          ...
          {error.split('\n').map((line, idx) => (
            line.trim() && (
              <p key={idx} className={`${line.startsWith('•')
      ...
      {loading && (
        <div className={`${isLight
          ...
          <Loader2 className={`w-6 h-6 ${isLight ? 'text-blue-600 animate-spin' : 'text-blue-400 animate-spin'
            }`} />
          <span className={`${isLight ? 'text-blue-600 font-semibold' : 'text-blue-400 font-semibold'
      ...
      {!loading && !error && predictions.length === 0 && searchQuery && (
        <div className={`${isLight
          ...
          <p className={`${isLight ? 'text-yellow-700' : isSpace ? 'text-yellow-300' : 'text-yellow-400'
      ...
      {predictions.length > 0 && (
        <div className={`${isLight
          ...
          <h3 className={`text-xl font-bold ${isLight ? 'text-gray-900' : isSpace ? 'text-white drop-shadow-lg' : 'text-white'
            } mb-4 flex items-center gap-2`}>
            <TrendingUp className={`w-6 h-6 ${isLight ? 'text-green-500' : isSpace ? 'text-green-400 drop-shadow' : 'text-green-400'
              }`} />
              ...
              return (
                <div
                  ...
                  className={`${isLight
                    ...
                    } transition-all ${isUnavailable ? 'opacity-60' : ''
                    }`}
                >
                  {isUnavailable ? (
                    <div>
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <p className={`${isLight ? 'text-gray-900' : isSpace ? 'text-white drop-shadow' : 'text-white'
                            } font-bold text-lg mb-1`}>{pred.symbol}</p>
                          <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${isLight ? 'bg-gray-100 text-gray-600' : 'bg-gray-500/20 text-gray-400'
                            }`}>
                            UNAVAILABLE
                          </span>
                        </div>
                      </div>
                      <div className={`p-3 rounded-lg ${isLight ? 'bg-yellow-50 border border-yellow-200' : 'bg-yellow-500/10 border border-yellow-500/30'
                        }`}>
                    ...
                    <>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          ...
                          <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${pred.action === 'LONG' ?
                        ...
                        {pred.current_price && !pred.trust_gate_active && (
                          <div>
                            <div className="flex justify-between">
                              <span className={`${isLight ? 'text-gray-600' : isSpace ? 'text-gray-300' : 'text-gray-400'
                                } text-sm`}>Model Reference Price:</span>
                              <span className={`${isLight ? 'text-gray-900 font-semibold' : isSpace ? 'text-white font-semibold' : 'text-white font-semibold'
                                }`}>{formatUSDToINR(pred.current_price, pred.symbol)}</span>
                            </div>
                            <p className={`text-xs mt-1 ${isLight ? 'text-gray-500' : isSpace ? 'text-gray-400' : 'text-gray-500'
                      ...
                      {pred.reason && (
                        <div className={`mt-3 pt-3 border-t ${isLight ? 'border-gray-300/50' : isSpace ? 'border-purple-800/50' : 'border-slate-600/50'
                          }`}>
                          <p className={`${isLight ? 'text-gray-700' : isSpace ? 'text-gray-300' : 'text-gray-300'
      ...
      {predictions.length > 0 && (
        <div className={`${isLight
          ...
          <h3 className={`text-xl font-bold ${isLight ? 'text-gray-900' : isSpace ? 'text-white drop-shadow-lg' : 'text-white'
            ...
            <BarChart3 className={`w-6 h-6 ${isLight ? 'text-blue-500' : isSpace ? 'text-blue-400 drop-shadow' : 'text-blue-400'
              }`} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className={`${isLight
              ...
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className={`w-5 h-5 ${isLight ? 'text-blue-500' : isSpace ? 'text-blue-400' : 'text-blue-400'
                  }`} />
                <h4 className={`font-semibold ${isLight ? 'text-gray-800' : isSpace ? 'text-white' : 'text-white'
                  }`}>Summary</h4>
              </div>
              <p className={`text-sm ${isLight ? 'text-gray-600' : isSpace ? 'text-gray-300' : 'text-gray-400'
            ...
            <div className={`${isLight
              ...
              <div className="flex items-center gap-2 mb-2">
                <BarChart3 className={`w-5 h-5 ${isLight ? 'text-green-500' : isSpace ? 'text-green-400' : 'text-green-400'
                  }`} />
                <h4 className={`font-semibold ${isLight ? 'text-gray-800' : isSpace ? 'text-white' : 'text-white'
                  }`}>Confidence</h4>
              </div>
            ...
            <div className={`${isLight
              ...
              <div className="flex items-center gap-2 mb-2">
                <Newspaper className={`w-5 h-5 ${isLight ? 'text-purple-500' : isSpace ? 'text-purple-400' : 'text-purple-400'
                  }`} />
                <h4 className={`font-semibold ${isLight ? 'text-gray-800' : isSpace ? 'text-white' : 'text-white'
                  }`}>Sentiment</h4>
              </div>
            ...
            <div className={`${isLight
              ...
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp className={`w-5 h-5 ${isLight ? 'text-yellow-500' : isSpace ? 'text-yellow-400' : 'text-yellow-400'
                  }`} />
                <h4 className={`font-semibold ${isLight ? 'text-gray-800' : isSpace ? 'text-white' : 'text-white'
                  }`}>Indicators</h4>
              </div>
          ...
          <div className={`mt-4 p-4 rounded-lg ${isLight ? 'bg-blue-50/50 border border-blue-300/30' : isSpace ? 'bg-blue-900/20 border border-blue-800/30' : 'bg-blue-500/10 border border-blue-500/30'
            }`}>
            <p className={`text-sm ${isLight ? 'text-blue-700' : isSpace ? 'text-blue-300' : 'text-blue-300'
...
const getActionIcon = (action: string, isLight?: boolean, isSpace?: boolean) => {
  switch (action?.toUpperCase()) {
    case 'LONG':
    case 'BUY':
      return <TrendingUp className={`w-5 h-5 ${isLight ? 'text-green-600' : isSpace ? 'text-green-400' : 'text-green-400'
        }`} />;
    case 'SHORT':
    case 'SELL':
      return <TrendingDown className={`w-5 h-5 ${isLight ? 'text-red-600' : isSpace ? 'text-red-400' : 'text-red-400'
        }`} />;
    case 'HOLD':
    default:
