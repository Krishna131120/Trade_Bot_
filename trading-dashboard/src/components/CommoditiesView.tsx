    ...
    <div className="space-y-4">
      <div>
        <h2 className={`text-xl font-bold ${isLight ? 'text-gray-900' : isSpace ? 'text-white drop-shadow-lg' : 'text-white'
          ...
          <Package className={`w-5 h-5 ${isLight ? 'text-orange-600' : isSpace ? 'text-orange-400 drop-shadow' : 'text-orange-400'
            }`} />
          Commodities Market
        </h2>
      ...
      <div className={`${isLight
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-3">
          <div className="flex-1 relative w-full">
            ...
            <input
              ...
              className={`w-full pl-8 pr-3 py-1.5 text-sm ${isLight
                ...
                {suggestions.map((symbol) => (
                  ...
                  >
                    ...
                    {commodityNames[symbol] && (
                      <span className={`text-xs ${isLight ? 'text-gray-500' : isSpace ? 'text-gray-400' : 'text-gray-500'
        ...
        <div>
          <p className={`${isLight ? 'text-gray-700' : isSpace ? 'text-gray-300' : 'text-gray-300'
            ...
            <BarChart3 className={`w-4 h-4 ${isLight ? 'text-orange-600' : isSpace ? 'text-orange-400 drop-shadow' : 'text-orange-400'
              }`} />
            Popular Commodities:
          </p>
      ...
      {error && (
        <div className={`${isLight
          ...
          <p className={`${isLight ? 'text-red-700' : isSpace ? 'text-red-300' : 'text-red-400'
            }`}>{error}</p>
        </div>
      )}
      ...
      {loading && (
        <div className={`${isLight
          ...
          <Loader2 className={`w-12 h-12 ${isLight ? 'text-orange-600 animate-spin' : 'text-orange-400 animate-spin'
            } mx-auto mb-4`} />
          <p className={`${isLight ? 'text-gray-600' : isSpace ? 'text-gray-300' : 'text-gray-400'
            }`}>Fetching commodities predictions from backend...</p>
          <p className={`${isLight ? 'text-gray-500 text-sm mt-2' : isSpace ? 'text-gray-400 text-sm mt-2' : 'text-gray-500 text-sm mt-2'
      ...
      {!loading && predictions.length === 0 && !error && (
        <div className={`${isLight
          ...
          <Package className={`w-16 h-16 ${isLight ? 'text-orange-600 mx-auto mb-4 opacity-50' : 'text-orange-400 mx-auto mb-4 opacity-50'
            }`} />
          ...
          <p className={`${isLight ? 'text-gray-600 mb-4' : isSpace ? 'text-gray-300 mb-4' : 'text-gray-400 mb-4'
        ...
        <div className={`${isLight
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {predictions.map((pred, index) => {
              ...
              return (
                <div key={index} className={`${isLight
                  ...
                  {isUnavailable ? (
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <span className={`font-bold text-lg block ${isLight ? 'text-gray-900' : isSpace ? 'text-white drop-shadow' : 'text-white'
                              }`}>{name}</span>
                            <span className={`text-xs ${isLight ? 'text-gray-600' : isSpace ? 'text-gray-400' : 'text-gray-400'
                              }`}>{symbol}</span>
                          </div>
                        </div>
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${isLight ? 'bg-gray-100 text-gray-600' : 'bg-gray-500/20 text-gray-400'
                          }`}>
                          UNAVAILABLE
                        </span>
                      </div>
                    ...
                    <>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${pred.action === 'LONG' ?
                      ...
                      <div className="space-y-1">
                        ...
                        <p className={`font-semibold ${isLight ? 'text-gray-900' : isSpace ? 'text-white' : 'text-white'
                          }`}>Predicted: ${pred.predicted_price?.toFixed(2) || 'N/A'}</p>
                        <p className={`text-sm font-semibold ${(pred.predicted_return || 0) > 0 ?
                        ...
                        <p className={`text-xs ${isLight ? 'text-gray-600' : isSpace ? 'text-gray-400' : 'text-gray-400'
